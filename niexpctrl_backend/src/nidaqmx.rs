...
extern "C" {
    ...
    fn DAQmxCfgSampClkTiming(handle: TaskHandle, src: CConstStr, rate: CFloat64, 
                                activeEdge: CInt32, sampleMode: CInt32, sampsPerChan: CUint64) -> CInt32;
    fn DAQmxCfgOutputBuffer(handle: TaskHandle, numSampsPerChan: CUint32) -> CInt32;

    fn DAQmxCreateAOVoltageChan(handle: TaskHandle, physical_name: CConstStr, assigned_name: CConstStr,
                                minVal: CFloat64, maxVal: CFloat64, units: CInt32, 
                                customScaleName: CConstStr) -> CInt32;
    fn DAQmxCreateDOChan(handle: TaskHandle, lines: CConstStr, name: CConstStr, lineGrouping: CInt32) -> CInt32;

    fn DAQmxWriteDigitalLines(handle: TaskHandle, seqLen: CInt32, autoStart: CBool32, 
                            timeout: CFloat64, dataLayout: CBool32, writeArray: *const u8, 
                            sampsPerChanWritten: *mut CInt32, reserved: *mut CBool32) -> CInt32;
    fn DAQmxWriteAnalogF64(handle: TaskHandle, seqLen: CInt32, autoStart: CBool32,
                            timeout: CFloat64, dataLayout: CBool32, writeArray: *const CFloat64,
                            sampsPerChanWritten: *mut CInt32, reserved: *mut CBool32) -> CInt32;
    ...
    fn DAQmxCfgDigEdgeStartTrig(handle: TaskHandle, triggerSource: CConstStr, triggerEdge: CInt32) -> CInt32;
...
fn daqmx_call<F: FnOnce() -> CInt32>(func: F) {
    ...
    if err_code < 0 {
        ...
        unsafe {DAQmxGetExtendedErrorInfo(err_buff.as_mut_ptr(), 2048 as CUint32);}
        ...
        let mut file = OpenOptions::new().write(true).append(true).create(true)
            .open("./nidaqmx_error.logs").expect("Failed to open nidaqmx_error.logs");
        writeln!(file, "DAQmx Error: {}", error_string).expect("Failed to write error to nidaqmx_error.logs");
...
impl NiTask {
    pub fn new() -> Self {
        ...
        let task_name_cstr = std::ffi::CString::new("").expect("Failed to convert task name to CString");
        daqmx_call(|| {unsafe {DAQmxCreateTask(task_name_cstr.as_ptr(), &mut taskhandle)}});
        Self {handle: taskhandle,}
    }

    pub fn clear(&self) {daqmx_call(|| {unsafe {DAQmxClearTask(self.handle)}});}
    pub fn start(&self) {daqmx_call(|| {unsafe {DAQmxStartTask(self.handle)}});}
    pub fn stop(&self) {daqmx_call(|| {unsafe {DAQmxStopTask(self.handle)}});}
    ...
    pub fn cfg_sample_clk(&self, clk_src: &str, samp_rate: f64, seq_len: u64) {
        let src_cstring = std::ffi::CString::new(clk_src).expect("Failed to convert clk_src to CString");
        daqmx_call(|| {unsafe {
            DAQmxCfgSampClkTiming(self.handle, src_cstring.as_ptr(), samp_rate as CFloat64,  
                                DAQMX_VAL_RISING, DAQMX_VAL_FINITESAMPS, seq_len as CUint64)
        }})
    }
    ...
    pub fn create_ao_chan(&self, physical_name: &str) {
        let physical_name_cstr = std::ffi::CString::new(physical_name).expect(
            "Failed to convert physical name to CString");
        let assigned_name_cstr = std::ffi::CString::new("").expect("");
        daqmx_call(|| {unsafe {
            DAQmxCreateAOVoltageChan(self.handle, physical_name_cstr.as_ptr(), assigned_name_cstr.as_ptr(),  
                                -10., 10., DAQMX_VAL_VOLTS, std::ptr::null())
        }})
    }
    ...
    pub fn create_do_chan(&self, physical_name: &str) {
        ...
        let assigned_name_cstr = std::ffi::CString::new("").expect("");
        daqmx_call(|| {unsafe {
            DAQmxCreateDOChan(self.handle, physical_name_cstr.as_ptr(), assigned_name_cstr.as_ptr(),  
                                DAQMX_VAL_CHANPERLINE)
        }})
    }

    pub fn write_digital(&self, signal_arr: &Array2<u8>) -> usize {
    ...
    pub fn set_ref_clk_src(&self, src: &str) {
        let clk_src_cstr = std::ffi::CString::new(src)
            .expect("Failed to convert ref_clk source to CString");
        daqmx_call(|| unsafe {
